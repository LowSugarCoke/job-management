...
import { fetchJobs, fetchJobById, createJob, updateJob, deleteJob } from './index';
...
describe('API tests', () => {
  ...
  test('fetchJobById should handle invalid data format', async () => {
    const invalidData = { id: 'wrong type' };
    mock.onGet(`http://localhost:8000/jobs/${jobData.id}`).reply(200, invalidData);

    await expect(fetchJobById(jobData.id)).rejects.toThrow(`Invalid job data format for job ID ${jobData.id}`);
  });
});
