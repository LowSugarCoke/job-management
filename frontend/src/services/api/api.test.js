...
/**
 * @summary Tests for job management API functions
 * @created by Jack Lee
 * @since 2024-06-07
 */
...
describe('API tests', () => {
  ...
  test('updateJob should return updated job data', async () => {
    ...
    mock
      .onPut(`http://localhost:8000/jobs/${jobData.id}`)
      .reply(200, updatedJobData)
  ...
  test('deleteJob should return deleted job data', async () => {
    mock
      .onDelete(`http://localhost:8000/jobs/${jobData.id}`)
      .reply(200, jobData)
  ...
  test('fetchJobs should handle invalid data format', async () => {
    const invalidData = [{ id: 'wrong type' }]
    mock.onGet('http://localhost:8000/jobs').reply(200, invalidData)

    await expect(fetchJobs()).rejects.toThrow('Invalid jobs data format')
  })

  test('fetchJobById should handle invalid data format', async () => {
    const invalidData = { id: 'wrong type' }
    mock
      .onGet(`http://localhost:8000/jobs/${jobData.id}`)
      .reply(200, invalidData)

    await expect(fetchJobById(jobData.id)).rejects.toThrow(
      `Invalid job data format for job ID ${jobData.id}`,
    )
  })
})
